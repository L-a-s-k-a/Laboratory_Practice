...
void GPIO_Ini_CMSIS(void)
{
    ...
    CLEAR_BIT(GPIOB->BSRR, GPIO_BSRR_BR7);//сброс BSRR, выключение светодиода 7-го пина GPIOB
    //set LED 4 (pin8)
    ...
    CLEAR_BIT(GPIOB->PUPDR, GPIO_PUPDR_PUPD8);//Отключение PU/PD резисторов для 8-го пина GPIOB
    CLEAR_BIT(GPIOB->BSRR, GPIO_BSRR_BR8);//сброс BSRR, выключение светодиода 8-го пина GPIOB
}