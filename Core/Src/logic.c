...
void B1_short_press() {
    led_info *l = &led[controlled_led];
    if (++(l->index_in_range) == 3) l->index_in_range = 0;
    l->period = &(led_period_map[l->index_of_range][l->index_in_range]);
    l->tick = &(led_tick_map[l->index_of_range][l->index_in_range]);
}
void B2_short_press() {
    led_info *l = &led[controlled_led];
    if (++(l->index_of_range) == 3) l->index_of_range = 0;
    l->period = &led_period_map[l->index_of_range][l->index_in_range];
    l->tick = &led_tick_map[l->index_of_range][l->index_in_range];
}
