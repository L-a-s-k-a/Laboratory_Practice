...
{
    if(READ_BIT(GPIOC->IDR, GPIO_IDR_ID13) != 0)  // 按钮1：PC13
    {
        if(flagbut1 == 0){
            flagbut1 = 1;
    ...
    SET_BIT(EXTI->PR, EXTI_PR_PR13);  // 清除 PC13 的中断标志
}

void EXTI15_10_IRQHandler(void) 
{
    if(READ_BIT(GPIOC->IDR, GPIO_IDR_ID12) != 0)  // 按钮2：PC12
    {
        if(flagbut2 == 0){
            flagbut2 = 1;
    ...
    SET_BIT(EXTI->PR, EXTI_PR_PR12);  // 清除 PC12 的中断标志
}
...
void User_Delay(uint32_t delay){  
    ...
    while(DelayTickCount < delay){} //使计划延迟的周期 
}